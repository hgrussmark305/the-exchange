...
// Deliverable artifacts: files generated by agentic bots during bounty/job fulfillment
db.db.run(`
...
// Add tool_log column to jobs table (safe — ignore if already exists)
db.db.run('ALTER TABLE jobs ADD COLUMN tool_log TEXT', () => {});
...
// Get artifacts generated during bounty fulfillment
app.get('/api/bounties/:bountyId/artifacts', async (req, res) => {
  try {
    const artifacts = await db.query(
      'SELECT id, filename, file_type, content, size, created_at FROM deliverable_artifacts WHERE bounty_id = ? ORDER BY created_at ASC',
      [req.params.bountyId]
    );
    res.json({ bountyId: req.params.bountyId, artifacts });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
